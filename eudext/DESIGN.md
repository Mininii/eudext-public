...
- **상수 제수가 2의 거듭제곱 2^s (2026-09-20)**: 나눗셈 본문을 아예 쓰지 않는다. i64 `_divmod_into` 에는 있던 접기가
  i128 에만 빠져 있어서 제수 16·4·2 가 `i64._cdiv_fn`("2의 거듭제곱 아님" 을 전제로 쓴 함수)으로 가고 있었다.
  - 나머지 = `a & (2^s − 1)` — 칸 하나만 `SetNumberX` 로 윗비트를 자르고 나머지 칸은 복사·0 (**호출 2 / 실행 3**).
  - 몫 = `a >> s`. s 가 32의 배수면 칸 옮기기뿐(**실행 3**). 아니면 겹친 창 (b_i, b_{i+1}) 을 i64 상수 시프트 본문
    `_shk_fn("shr", s%32)` 으로 세 번 — 위 두 칸이 0 이면(값 < 2^64) 창 하나로 끝내는 빠른 길(`_cdiv_fn` 과 같은 모양).
    **실행 73~211**(기존 40~212), 굽는 본문 139 → 63(i64 와 공유, 제수마다 약 −27KB), 사본마다 +1.4KB.
  - 즉 **나머지·32배수 시프트는 완승, 몫은 맞바꾸기**다(사본 약 18곳이 손익분기). 느린 쪽 원인은 i64 의 작은 n
    오른쪽 상수 시프트(`_shr_k_emit` 비트 옮기기 61트리거)이고, 그것을 빠르게 하면 같이 빨라진다(후속 후보).
- **`CONST_DIVISOR` 용량 스위치 (2026-09-20, `i64`·`i128` 각각)**: 상수 제수 전용 본문을 굽지 않고 변수 제수 공유
  본문으로 보낸다. 기본 `True`(빠른 쪽 — 3.8 성능 우선). 끄면 상수 종류마다 i64 약 36KB / i128 약 47KB 가 통째로
  사라지는 대신 실행이 3~4배(i128 40~215 → 72~680, i64 12~79 → 26~314). 2의 거듭제곱 접기는 이 스위치와 무관하게
  늘 한다(더 싸다). `tools/size_report.py --off i64.CONST_DIVISOR` 로 두 번 재서 견준다(5.7).
- **접는 나눗셈(`unroll=False`)은 넣지 않았다 (2026-09-20 조사)**: DPS `war.py._emit_steps(unroll=False)` 는 단
  하나를 루프로 돈다 — DPS 는 펼친 판이 3,331 트리거(8.0MB)였기 때문이다. eudext 의 `_d128_emit` 은 이미 자기수정
  다음포인터로 **64단을 단당 트리거 2개**(본문 209 전체)로 낸다 — DPS 펼친 판의 1/16 이다. 여기서 더 접으려면
  피제수를 미는 다른 식(DPS 꼴)으로 갈아야 하고, 얻는 것은 본문 209 → 약 90(약 −210KB, 128비트 제수를 쓰는 맵에
  한해)인데 실행이 최악 1,147 → 약 1,660 으로 는다. **성능 우선 원칙에 걸려 기본값으로는 못 넣고**, 선택 스위치로
  넣기에는 두 번째 알고리즘을 통째로 안고 가야 해서 값이 안 맞는다고 봤다. 대신 같은 목적(용량 급할 때 트리거를
  포기)은 위 `CONST_DIVISOR` 스위치가 훨씬 싼 값에 준다(상수마다 36~47KB).
...
### 5.7 `tools/size_report.py` — 페이로드를 만든 자리별로 (2026-09-20)

`cost.py`(5.2)가 **항목 하나**를 잰다면 이쪽은 **맵 하나를 통째로 빌드해 놓고** 페이로드 객체를 만든 파이썬 호출 자리로
되짚어 파일별·줄별로 합친다. 어느 모듈·어느 줄이 용량을 먹는지 한 장에서 본다.
DPS `tools/size_report.py`(Lua 스택에서 자리를 찾는 판)를 파이썬 프레임으로 옮긴 것이다.

    python tools/size_report.py <플러그인.py|.eps ...> [--sites N] [--files N] [--attribute eudext|caller] [--off 모듈.스위치]

- **되짚는 법**: 빌드 동안 `EUDObject` 갈래들의 `__init__` 을 감싸 호출 자리를 적는다. eudplib 0.81 의 `EUDObject`
  는 Rust 쪽 `__new__` 라 그쪽을 감싸면 안전성 검사에 걸린다 — `__init__` 을 쓴다. eudplib 은 **SaveMap 때** 본문을
  굽기 때문에 감싸기는 `SaveMap` 을 감싼 동안에만 걸고 끝나면 원래대로 되돌린다(`t_size_report.py` 가 되돌림을 본다).
- 자리 = 스택에서 건너뛸 꾸러미(eudplib, 이 도구) 밖의 첫 프레임. `--attribute caller` 면 eudext 도 건너뛰어 맵 코드로
  몰아 센다. 맵·eudext 프레임이 하나도 없으면(eudplib 이 스스로 굽는 도우미 본문) `[eudplib] <파일>` 로 갈래를 보여 준다.
- `--off i64.CONST_DIVISOR,i128.CONST_DIVISOR` 처럼 **용량 스위치를 끄고 두 번 재서 견준다**(3.8 "용량 작업은 실측
  절감치와 런타임 대가를 같이"). 스위치 목록은 COSTS.md 2026-09-20 절.
- 파이썬에서도 쓴다: `size_report.measure(save, attribute=…)` → `Report`(by_class·by_file·by_site·total), `render(rep, sites=N)`.
