...
**2026-09-20 에 보탠 세 가지는 아직 확인 맵이 없다 — 인게임 미확인**(에뮬레이터 차등만 통과, `tests/t_spawn_p6c.py`):

| # | 항목 | 어떻게 볼까 | 결과 |
|---|---|---|---|
| E12-10 | `rot3d`(`push(rot3d=True)` + `sp.rot3d`) 가 원본 `CA_Rotate3D` 처럼 보이는가 — 각을 매 프레임 올리면 도형이 **누운 원**(타원)으로 돌아야 한다 | 확인 맵에 단계를 보태거나, **Mem2 P6-c** 이식판에서 3D 태그 59곳이 원본과 같은 모양인지로 갈음한다. `mathx.rotate3d` 자체는 C9-2 에서 통과했다 | **미확인** |
| E12-11 | `size_div=256`(`Spawner(size_div=256)`) 이 원본 `CA_RatioXY(v,256,v,256)` 와 같은 크기인가 | 같은 도형을 `size=352`(137.5%) 로 찍어 원본과 겹쳐 본다. Mem2 의 26곳이 그 자리다 | **미확인** |
| E12-13 | `precise=True`(고정밀 lengthdir 표) 회전이 원본(`Include_CtrigPlib(…, LengthdirX=1)` 을 켠 맵 — Mem2)과 같은가 | 같은 도형을 큰 반지름으로 여러 각에 돌려 원본과 겹쳐 본다. `mathx` 의 고정밀 표 자체는 C9 에서 봤다 | **미확인** |
| E12-12 | `bounds="clamp"` 가 원본 `CA_Func` 처럼 **맵 밖 점을 가장자리에 몰아** 찍는가(버리지 않는다) | E12-1 과 같은 자리(오른쪽 끝·아래 끝)에서 `bounds="clamp"` 로 찍으면 41점이 **하나도 안 빠지고** 가장자리에 겹쳐 쌓여야 한다. 다만 게임이 겹친 유닛을 밀어낸다(D7-1 과 같은 사정) | **미확인** |
